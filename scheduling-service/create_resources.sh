...
FUNC_NAME=${FUNC_NAME:-scheduling-service}
DATABASE_URL=${DATABASE_URL:-postgres://admin:secret@db:5432/scheduling_service?sslmode=disable}
...
# ===== Ensure /scheduling resource =====
SCHED_ID=$(get_resource_id_by_path "$API_ID" "/scheduling")
if [[ -z "${SCHED_ID// }" || "$SCHED_ID" == "None" ]]; then
  echo "==> Creating resource '/scheduling'..."
  SCHED_ID=$(aws --region "$AWS_DEFAULT_REGION" apigateway create-resource \
    --rest-api-id "$API_ID" \
    --parent-id "$ROOT_ID" \
    --path-part scheduling \
    --query 'id' \
    --output text \
    --endpoint-url "$LS_ENDPOINT")
else
  echo "==> Resource '/scheduling' exists: $SCHED_ID"
fi

# ===== Ensure /scheduling/{proxy+} resource =====
SCHED_PROXY_ID=$(get_resource_id_by_path "$API_ID" "/scheduling/{proxy+}")
if [[ -z "${SCHED_PROXY_ID// }" || "$SCHED_PROXY_ID" == "None" ]]; then
  echo "==> Creating resource '/scheduling/{proxy+}'..."
  SCHED_PROXY_ID=$(aws --region "$AWS_DEFAULT_REGION" apigateway create-resource \
    --rest-api-id "$API_ID" \
    --parent-id "$SCHED_ID" \
    --path-part "{proxy+}" \
    --query 'id' \
  ...
  echo "==> Resource '/scheduling/{proxy+}' exists: $SCHED_PROXY_ID"
fi
...
# ===== Put ANY method + integration on /scheduling =====
echo "==> Configuring ANY on '/scheduling'..."
aws --region "$AWS_DEFAULT_REGION" apigateway put-method \
  --rest-api-id "$API_ID" \
  --resource-id "$SCHED_ID" \
  --http-method ANY \
...
# ===== Put ANY method + integration on /scheduling/{proxy+} =====
echo "==> Configuring ANY on '/scheduling/{proxy+}'..."
aws --region "$AWS_DEFAULT_REGION" apigateway put-method \
  --rest-api-id "$API_ID" \
  --resource-id "$SCHED_PROXY_ID" \
  --http-method ANY \
  --authorization-type NONE \
  --no-api-key-required \
...
aws --region "$AWS_DEFAULT_REGION" lambda add-permission \
  --function-name "$FUNC_NAME" \
  --statement-id "apigw-scheduling-root" \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "arn:aws:execute-api:$AWS_DEFAULT_REGION:000000000000:$API_ID/*/*/scheduling" \
  --endpoint-url "$LS_ENDPOINT" >/dev/null 2>&1 || true

aws --region "$AWS_DEFAULT_REGION" lambda add-permission \
  --function-name "$FUNC_NAME" \
  --statement-id "apigw-scheduling-proxy" \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "arn:aws:execute-api:$AWS_DEFAULT_REGION:000000000000:$API_ID/*/*/scheduling/*" \
  --endpoint-url "$LS_ENDPOINT" >/dev/null 2>&1 || true
...
echo ""
echo "Scheduling routes (examples):"
echo "  POST $LS_ENDPOINT/restapis/$API_ID/$STAGE/_user_request_/scheduling/create"
echo ""
echo "Test (create):"
echo "  curl -i -X POST \\"
echo "    $LS_ENDPOINT/restapis/$API_ID/$STAGE/_user_request_/scheduling/create \\"
